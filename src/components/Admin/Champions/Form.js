...
export default class Form extends Component {
    ...
    componentDidMount() {
        try {
            ...
            axios.get(`http://localhost:3000/champions/g/${champion}`)
                .then(champion => {
                    this.setState({...champion.data.collection, championReady: true})
                })
    ...
    uploadMedia = (event, type, typePath = '', num) => {
        ...
        const token = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyRGF0YSI6eyJpZCI6IjViMjYxZjBkZjI5YjhhNTZhNDJlMGEwYyIsIm5hbWUiOiJBYmRlbHJobWFuIG1vaGFtZWQiLCJlbWFpbCI6ImFiZGVscmhtYW5AY29kZXJzZWEuY29tIiwicm9sZSI6ImFkbWluIn0sImlhdCI6MTUzMTY3NDc5NywiZXhwIjoxNTMzNDAyNzk3fQ.0gk9j0RRBwdJlZ0_QkNoPtyMyZndKUkTqASKumESTNw"
        let config = {"Content-Type": "multipart/form-data", authorization: token};
        axios.post(`http://localhost:3000/upload/${champion}/${type}`,
            data, config)
            .then(docs => {
                if (num !== undefined) {
                    switch (type) {
                        case 'video': // Combos
                            ...
                            combos[num].video = docs.data.path
                            this.setState({combos})
                            break
                        case 'icon': // Spells & battlerites
                            if (typePath === '/spells') {
                                let spells = this.state.spells
                                spells[num].img = docs.data.path
                                this.setState({spells})

                                document.getElementById(`spellIconLabel-${num}`).innerText =
                                    document.getElementById(`spellIcon-${num}`).files[0].name
                            } else if (typePath === '/battlerites') {
                                let battlerites = this.state.battlerites
                                battlerites[num].img = docs.data.path
                                this.setState({battlerites})

                                document.getElementById(`battleriteIconLabel-${num}`).innerText =
                                    document.getElementById(`battleriteIcon-${num}`).files[0].name
                            }
                            break
                        case 'sound': // Quotes
                            let quotes = this.state.quotes
                            quotes[num].sound = docs.data.path
                    ...
                    switch (type) {
                        case 'video':
                            this.setState({video: docs.data.path})

                            document.getElementById("champUploadVideoLabel").innerText =
                                document.getElementById("champUploadVideo").files[0].name
                            break
                        case 'icon':
                            this.setState({icon: docs.data.path})

                            document.getElementById("champUploadIconLabel").innerText =
                                document.getElementById("champUploadIcon").files[0].name
                            break
                        case 'avatar':
                            this.setState({avatar: docs.data.path})

                            document.getElementById("champUploadAvatarLabel").innerText =
                                document.getElementById("champUploadAvatar").files[0].name
                            break
                        default:
                            //
                            break
    ...
    render() {
        return (
            <div className="main-content aChampions col-12 py-4">
                {this.state.championReady ? <form onSubmit={e => e.preventDefault()}>
                                ...
                                <div className="col-lg-6">
                                    <div className="card bg-dark border border-secondary mt-3 text-light">
                                        <div className="card-body">
                                            ...
                                            <div className="d-flex flex-column">
                                                <DefaultInput id="champBasicGuideTitle"
                                                ...
                                                return <div className={`tab-pane fade ${key === 0 ? 'show active' : ''}`}
                                                    ...
                                                    <div className="d-flex">
                                                        ...
                                                        <div className="mb-0 ml-4 mt-3">
                                                            <DefaultInput id={`champStatusRate-${status.name}`}
                                                                          value={status.rate}
                                                                          onChangeValue={value => this.setStatus(value, key, 'rate')}
                                                                          label="Rate" type="number" min="0" max="100"
                                                                          step="5"/>
                                                        </div>
                                                    </div>
                            ...
                            <div className="row mt-4">
                                ...
                                {this.state.battlerites.map((battlerite, key) => {
                                    return <div className="col-6 mt-3" key={key}>
                                        <div className="card border-light rounded bg-dark text-light">
                                            <div className="card-body">
                                                <DefaultInput id={`battleriteName-${key}`} required={true}
                                                              value={battlerite.name}
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="col-6 d-flex">
                                                        <DefaultInput id={`battleriteKeyword-${key}`}
                                                                      value={battlerite.keyword}
